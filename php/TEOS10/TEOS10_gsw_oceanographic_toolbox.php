...
* This is as translation of the C, F90 into PHP for web-based applications.
* The original notices follow.
* Translation by Susanne Feistel, IOW 11/07/2013, 30/05/2014
...
! Authors(Matlab): Trevor McDougall and Paul Barker        [ help@teos-10.org ]
!==========================================================================
!
...
class TEOS10_gsw_oceanographic_toolbox {
    # functions
    public $GSW_saar;
    ...
    */
    function gsw_sp_from_c($c,$t,$p) {
        ...
        $sp = ''; $t68 = ''; $ft68 = ''; $r = ''; $rt_lc = ''; $rp = ''; $rt = ''; $rtx = '';
        $hill_ratio = ''; $x = ''; $sqrty = ''; $part1 = ''; $part2 = ''; $sp_hill_raw = '';
        ...
        if ($sp < 2) {
            $hill_ratio = self::gsw_hill_ratio_at_sp2($t);
            $x = 400e0*$rt;
            $sqrty = 10e0*$rtx;
            $part1 = 1e0 + $x*(1.5e0 + $x);
    ...
    */
    function gsw_c_from_sp($sp,$t,$p) {
        ...
        // to Rtx.
        //--------------------------------------------------------------------------
        $dsp_drtx =  $a1 + (2e0*$a2 + (3e0*$a3 + (4e0*$a4 + 5e0*$a5*$rtx)*$rtx)*$rtx)*$rtx
        + $ft68*($b1 + (2e0*$b2 + (3e0*$b3 + (4e0*$b4 + 5e0*$b5*$rtx)*$rtx)*$rtx)*$rtx);
        ...
        if ($sp < 2) {
            ...
            + $b0*$ft68*(10e0 + $sqrty*(20e0 + 30e0*$sqrty))/($part2*$part2);
        ...
        $dsp_drtx =  $a1 + (2e0*$a2 + (3e0*$a3 + (4e0*$a4 + 5e0*$a5*$rtxm)*$rtxm)*$rtxm)*$rtxm
        + $ft68*($b1 + (2e0*$b2 + (3e0*$b3 + (4e0*$b4 + 5e0*$b5*$rtxm)*$rtxm)*$rtxm)*$rtxm);
        if ($sp_est < 2) {
            $x = 400e0*($rtxm*$rtxm);
            $sqrty = 10e0*$rtxm;
            $part1 = 1e0 + $x*(1.5e0 + $x);
        ...
        $cc  = $p*($e1 + $p*($e2 + $e3*$p));
        // rt_lc (i.e. rt_lower_case) corresponds to rt as defined in the UNESCO 44 (1983) routines.
        $rt_lc = $c0 + ($c1 + ($c2 + ($c3 + $c4*$t68)*$t68)*$t68)*$t68;
    ...
    */
    function gsw_sa_from_sp($sp, $p, $lon, $lat) {
        ...
        $saar = $this->GSW_saar->gsw_saar($p,$lon,$lat);

        if ($saar == GSW_INVALID_VALUE)
            return ($saar);

        $gsw_sa_baltic	= self::gsw_sa_from_sp_baltic($sp,$lon,$lat);
    ...
    */
    function gsw_sstar_from_sp($sp, $p, $lon, $lat)	{
        ...
        */
        $sstar_baltic	= self::gsw_sa_from_sp_baltic($sp,$lon,$lat);
    ...
    */
    function gsw_ct_from_t($sa, $t, $p) {
        ...
        return (self::gsw_ct_from_pt($sa,$pt0));
    }
    ...
    */
    function gsw_deltasa_from_sp($sp, $p, $lon, $lat) {
        ...
        $res	= self::gsw_sa_from_sp($sp,$p,$lon,$lat) - self::gsw_sr_from_sp($sp);
        if ($res > 1e10)
    ...
    */
    function gsw_sp_from_sa($sa, $p, $lon, $lat) {
        ...
        if ($saar == GSW_INVALID_VALUE)
            return ($saar);
        $gsw_sp_baltic	= self::gsw_sp_from_sa_baltic($sa,$lon,$lat);
    ...
    */
    function gsw_sp_from_sstar($sstar, $p, $lon, $lat) {
        ...
        */
        $sp_baltic	= self::gsw_sp_from_sa_baltic($sstar,$lon,$lat);
        if ($sp_baltic < 1810)
            return ($sp_baltic);
    ...
    */
    function gsw_t_from_ct($sa, $ct, $p) {
        ...
        $p0	= 0e0;
        $pt0 = self::gsw_pt_from_ct($sa,$ct);
        return (self::gsw_pt_from_t($sa,$pt0,$p0,$p));
    }
    ...
    */
    function gsw_pt_from_t($sa, $t, $p, $p_ref) {
        ...
        $true_entropy_part	= self::gsw_entropy_part($sa,$t,$p);
        for ($no_iter=1; $no_iter <= 2; $no_iter++) {
            $pt_old		= $pt;
            $dentropy	= self::gsw_entropy_part($sa,$pt_old,$p_ref) - $true_entropy_part;
            $pt			= $pt_old - $dentropy/$dentropy_dt;
            $ptm		= 0.5e0*($pt + $pt_old);
            $dentropy_dt= -1* self::gsw_gibbs($n0,$n2,$n0,$sa,$ptm,$p_ref);
            $pt			= $pt_old - $dentropy/$dentropy_dt;
        }
        return ($pt);
    }
    ...
    */
    function gsw_pt0_from_t($sa, $t, $p) {
        ...
        for ($no_iter=1; $no_iter <= 2; $no_iter++) {
            $pt0_old	= $pt0;
            $dentropy	= self::gsw_entropy_part_zerop($sa,$pt0_old) - $true_entropy_part;
            $pt0		= $pt0_old - $dentropy/$dentropy_dt;
            $pt0m		= 0.5e0*($pt0 + $pt0_old);
            $dentropy_dt= -1 * self::gsw_gibbs_pt0_pt0($sa,$pt0m);
            $pt0		= $pt0_old - $dentropy/$dentropy_dt;
        }
        return ($pt0);
    }
    ...
    */
    function gsw_pt_from_ct($sa, $ct) {
        ...
        $a5ct = ''; $b3ct = ''; $ct_factor = ''; $pt_num = ''; $pt_den = ''; $ct_diff = '';
        $pt = ''; $pt_old = ''; $ptm = ''; $dct_dpt = '';
        ...
        */
        ...
        $ct_diff	= self::gsw_ct_from_pt($sa,$pt) - $ct;
        $pt_old		= $pt;
        $pt			= $pt_old - ($ct_diff)/$dct_dpt;
        $ptm		= 0.5e0*($pt + $pt_old);

        $dct_dpt	= -1*($ptm + 273.15e0) * self::gsw_gibbs_pt0_pt0($sa,$ptm)/$cp0;
    ...
    */
    function gsw_z_from_p($p, $lat) {
        $pi = 3.141592653589793e0; // php constant M_PI has only 14 digits (per default in php.ini)

        $gamma = ''; $deg2rad = ''; $x = ''; $sin2 = ''; $b = ''; $c = ''; $a = '';
    ...
    */
    function gsw_entropy_from_t($sa, $t, $p) {
        ...
        $n0    = 0;
        $n1    = 1;
        return (-1 * self::gsw_gibbs($n0,$n1,$n0,$sa,$t,$p));
    }
    ...
    */
    function gsw_adiabatic_lapse_rate_from_ct($sa, $ct, $p) {
        ...
        $pr0   = 0e0;
        $pt0   = self::gsw_pt_from_ct($sa,$ct);
        $t     = self::gsw_pt_from_t($sa,$pt0,$pr0,$p);

        return (-1 * self::gsw_gibbs($n0,$n1,$n1,$sa,$t,$p)/(self::gsw_gibbs($n0,$n2,$n0,$sa,$t,$p)));
    ...
    */
    function gsw_rho($sa, $ct, $p) {
        ...
        $v01 = 9.998420897506056e+2;        $v02 =  2.839940833161907e0;
        ...
        $v21 =  1.0e0;                      $v22 =  2.775927747785646e-3;
        $v23 = -2.349607444135925e-5;       $v24 =  1.119513357486743e-6;
        $v25 =  6.743689325042773e-10;      $v26 = -7.521448093615448e-3;
        $v27 = -2.764306979894411e-5;       $v28 =  1.262937315098546e-7;
        $v29 =  9.527875081696435e-10;      $v30 = -1.811147201949891e-11;
        $v31 = -3.303308871386421e-5;       $v32 =  3.801564588876298e-7;
        $v33 = -7.672876869259043e-9;       $v34 = -4.634182341116144e-11;
        $v35 =  2.681097235569143e-12;      $v36 =  5.419326551148740e-6;
        $v37 = -2.742185394906099e-5;       $v38 = -3.212746477974189e-7;
        $v39 =  3.191413910561627e-9;       $v40 = -1.931012931541776e-12;
        $v41 = -1.105097577149576e-7;       $v42 =  6.211426728363857e-10;
        $v43 = -1.119011592875110e-10;      $v44 = -1.941660213148725e-11;
        $v45 = -1.864826425365600e-14;      $v46 =  1.119522344879478e-14;
        $v47 = -1.200507748551599e-15;      $v48 =  6.057902487546866e-17;
    ...
    */
    function gsw_alpha($sa, $ct, $p) {
        ...
        return (($v_hat_denominator*$dvhatnum_dct-$v_hat_numerator*$dvhatden_dct)/($v_hat_numerator*$v_hat_denominator));
    }
    ...
    */
    function gsw_beta($sa, $ct, $p) {
        ...
        return (($v_hat_numerator*$dvhatden_dsa-$v_hat_denominator*$dvhatnum_dsa)/($v_hat_numerator*$v_hat_denominator));
    ...
    */
    function gsw_alpha_on_beta($sa, $ct, $p) {
        $v01 =  9.998420897506056e2;  $v02 =  2.839940833161907e0;
        $v03 = -3.147759265588511e-2; $v04 =  1.181805545074306e-3;
        $v05 = -6.698001071123802e0;  $v06 = -2.986498947203215e-2;
        $v07 =  2.327859407479162e-4; $v08 = -3.988822378968490e-2;
        $v09 =  5.095422573880500e-4; $v10 = -1.426984671633621e-5;
        ...
        $sqrtsa = ''; $v_hat_denominator = ''; $v_hat_numerator = '';
        $dvhatden_dct = ''; $dvhatnum_dct = '';
        $dvhatden_dsa = ''; $dvhatnum_dsa = '';
    ...
    */
    ...
    function gsw_rho_first_derivatives($sa,$ct,$p,&$drho_dsa,&$drho_dct,&$drho_dp) {
        ...
        $v01 =  9.998420897506056e+2;        $v02 =  2.839940833161907;
        $v03 = -3.147759265588511e-2;        $v04 =  1.181805545074306e-3;
        $v05 = -6.698001071123802;           $v06 = -2.986498947203215e-2;
        $v07 =  2.327859407479162e-4;        $v08 = -3.988822378968490e-2;
        $v09 =  5.095422573880500e-4;        $v10 = -1.426984671633621e-5;
        $v11 =  1.645039373682922e-7;        $v12 = -2.233269627352527e-2;
        $v13 = -3.436090079851880e-4;        $v14 =  3.726050720345733e-6;
        ...
        $a01 =  2.839940833161907;           $a02 = -6.295518531177023e-2;
        $a03 =  3.545416635222918e-3;        $a04 = -2.986498947203215e-2;
        $a05 =  4.655718814958324e-4;        $a06 =  5.095422573880500e-4;
        $a07 = -2.853969343267241e-5;        $a08 =  4.935118121048767e-7;
        $a09 = -3.436090079851880e-4;        $a10 =  7.452101440691467e-6;
        ...
        $b01 = -6.698001071123802;           $b02 = -2.986498947203215e-2;
        $b03 =  2.327859407479162e-4;        $b04 = -5.983233568452735e-2;
        $b05 =  7.643133860820750e-4;        $b06 = -2.140477007450431e-5;
        $b07 =  2.467559060524383e-7;        $b08 = -1.806789763745328e-4;
        $b09 =  6.876837219536232e-7;        $b10 =  1.550932729220080e-10;
        ...
        $sqrtsa = ''; $v_hat_denominator = ''; $v_hat_numerator = '';
        $dvhatden_dct = ''; $dvhatnum_dct = ''; $dvhatden_dsa = ''; $dvhatnum_dsa = '';
        $dvhatden_dp = ''; $dvhatnum_dp = ''; $rho = ''; $rec_num = '';
        ...
        $drho_dp = self::pa2db*($dvhatden_dp - $dvhatnum_dp*$rho)*$rec_num;
    }
    ...
    */
    function gsw_specvol($sa, $ct, $p) {
        return (1e0/self::gsw_rho($sa,$ct,$p));
    }
    ...
    */
    function gsw_specvol_anom($sa, $ct, $p) {
        return (self::gsw_specvol($sa,$ct,$p) - self::gsw_specvol_sso_0_p($p));
    }
    ...
    */
    function gsw_sigma0($sa, $ct) {
        ...
        $v_hat_denominator = ''; $v_hat_numerator = ''; $sqrtsa = '';
    ...
    */
    function gsw_sigma1($sa, $ct) {
        return (self::gsw_rho($sa,$ct,1000e0) - 1000e0);
    }
    ...
    * ct     : Conservative Temperature                        [deg C]
    *
    * gsw_sigma1  : potential density anomaly with reference pressure of 2000
    *                                                      (48 term equation)
    */
    function gsw_sigma2($sa, $ct) {
        return (self::gsw_rho($sa,$ct,2000e0) - 1000e0);
    }
    ...
    * ct     : Conservative Temperature                        [deg C]
    *
    * gsw_sigma1  : potential density anomaly with reference pressure of 3000
    *                                                      (48 term equation)
    */
    function gsw_sigma3($sa, $ct) {
        return (self::gsw_rho($sa,$ct,3000e0) - 1000e0);
    }
    ...
    * ct     : Conservative Temperature                        [deg C]
    *
    * gsw_sigma1  : potential density anomaly with reference pressure of 4000
    *                                                      (48 term equation)
    */
    function gsw_sigma4($sa, $ct) {
        return (self::gsw_rho($sa,$ct,4000e0) - 1000e0);
    }
    ...
    */
    function gsw_kappa($sa, $ct, $p) {
        ...
        $v_hat_denominator = ''; $v_hat_numerator = '';
        $sqrtsa = ''; $dvden_dp = ''; $dvnum_dp = '';
    ...
    */
    function gsw_cabbeling($sa, $ct, $p) {
        ...
        $sqrtsa = ''; $v_hat_denominator = ''; $v_hat_numerator = '';
        $dvhatden_dct = ''; $dvhatnum_dct = ''; $dvhatden_dctdct = ''; $dvhatden_dctdsa = '';
        $dvhatden_dsa = ''; $dvhatnum_dsa = ''; $dvhatnum_dsadsa = ''; $dvhatden_dsadsa = '';
        $dvhatnum_dctdct = ''; $dvhatnum_dsadct = ''; $dvhatnum_dctdsa = '';
        $p1a = ''; $p1b = ''; $p1c = ''; $p1d = ''; $part1 = ''; $factor2a = ''; $factor2b = '';
        $p2a = ''; $p2b = ''; $p2c = ''; $p2d = ''; $p2e = ''; $part2 = '';
        $factor3a = ''; $factor3b = ''; $p3a = ''; $p3b = ''; $p3c = ''; $p3d = ''; $part3 = '';
    ...
    */
    function gsw_thermobaric($sa, $ct, $p) {
        ...
        $sqrtsa = ''; $v_hat_denominator = ''; $v_hat_numerator = '';
        $dvhatden_dct = ''; $dvhatnum_dct = ''; $dvhatden_dp = ''; $dvhatnum_dp = '';
        $dvhatden_dsa = ''; $dvhatnum_dsa = ''; $dvhatden_dpdct = ''; $dvhatnum_dpdct = '';
        $dvhatden_dpdsa = ''; $dvhatnum_dpdsa = '';
        $p1a = ''; $p1b = ''; $p1c = ''; $p1d = ''; $p1e = ''; $part1 = ''; $factor2 = '';
        $p2a = ''; $p2b = ''; $p2c = ''; $p2d = ''; $p2e = ''; $part2 = '';
    ...
    */
    function gsw_internal_energy($sa, $ct, $p) {
        return (self::gsw_enthalpy($sa,$ct,$p) - (self::p0 + self::db2pa*$p)*self::gsw_specvol($sa,$ct,$p));
    }
    ...
    */
    function gsw_enthalpy($sa, $ct, $p) {
        ...
        $cp0		= self::GSW_cp0;   /* from Eqn. (3.3.3) of IOC et al. (2010) */
        ...
        return 	($cp0*$ct + self::db2pa*($p*($a2 - 2.0*$a3*$b1/$b2 + 0.5*$a3*$p)/$b2 +
    ...
    */
    function gsw_dynamic_enthalpy($sa, $ct, $p) {
        ...
        return 		(self::db2pa *($p*($a2 - 2.0*$a3*$b1/$b2 + 0.5*$a3*$p)/$b2
    ...
    * sa  =  Absolute Salinity                                        [g/kg]
    *
    * Coded from the Fortran version by Bernard Saulmé
    */
    function gsw_sa_from_rho($rho, $ct, $p) {
        $no_iter= '';
        $sa= ''; $v_lab= ''; $v_0= ''; $v_50= ''; $v_sa= '';
        $sa_old= ''; $delta_v= ''; $sa_mean= ''; $alpha= ''; $beta= '';

        $v_lab    = 1e0/$rho;
        $v_0    = self::gsw_specvol(0e0,$ct,$p);
        $v_50    = self::gsw_specvol(50e0,$ct,$p);
        ...
        for ($no_iter=1; $no_iter <= 2; $no_iter++) {
            $sa_old    = $sa;
            $delta_v    = self::gsw_specvol($sa_old,$ct,$p) - $v_lab;
            $sa        = $sa_old - $delta_v/$v_sa;
            $sa_mean    = 0.5e0*($sa + $sa_old);
            $alpha    = self::gsw_alpha($sa_mean,$ct,$p);
            $beta    = self::gsw_beta($sa_mean,$ct,$p);
            $v_sa    = - $beta/$rho;
            $sa        = $sa_old - $delta_v/$v_sa;
            if ($sa < 0e0 || $sa > 50e0)
                $sa = GSW_INVALID_VALUE;
    ...
    */
    ...
    function gsw_nsquared(&$sa, &$ct, &$p, &$lat, $nz, &$n2, &$p_mid) {
        ...
        $db2pa = self::db2pa;
        $grav_local = ''; $dsa = ''; $sa_mid = ''; $dct = ''; $ct_mid = '';
        $dp = ''; $rho_mid = ''; $alpha_mid = ''; $beta_mid = '';


        for ($k = 0; $k < $nz-1; $k++) {
            $grav_local = 0.5*(self::gsw_grav($lat[$k],$p[$k]) + self::gsw_grav($lat[$k+1],$p[$k+1]));
            ...
            $rho_mid = self::gsw_rho($sa_mid,$ct_mid,$p_mid[$k]);
            $alpha_mid = self::gsw_alpha($sa_mid,$ct_mid,$p_mid[$k]);
            $beta_mid = self::gsw_beta($sa_mid,$ct_mid,$p_mid[$k]);
    ...
    */
    ...
    function gsw_turner_rsubrho(&$sa, &$ct, &$p, $nz,&$tu, &$rsubrho, &$p_mid) {
        ...
        $dsa = ''; $sa_mid = ''; $dct = ''; $ct_mid = '';
        $dp = ''; $alpha_mid = ''; $beta_mid = '';
    ...
    */
    /* void */
    function gsw_ipv_vs_fnsquared_ratio(&$sa, &$ct, &$p, $nz, &$ipv_vs_fnsquared_ratio, &$p_mid) {

        $k = '';
        $dsa = ''; $sa_mid = ''; $dct = ''; $ct_mid = ''; $dp = ''; $p_ref = '';
        $alpha_mid = ''; $beta_mid = '';
        $alpha_pref = ''; $beta_pref = ''; $numerator = ''; $denominator = '';
        ...
        for ($k = 0; $k < $nz-1; $k++) {
            ...
            $alpha_mid = self::gsw_alpha($sa_mid,$ct_mid,$p_mid[$k]);
            $beta_mid = self::gsw_beta($sa_mid,$ct_mid,$p_mid[$k]);
            $alpha_pref = self::gsw_alpha($sa_mid,$ct_mid,$p_ref);
            $beta_pref = self::gsw_beta($sa_mid,$ct_mid,$p_ref);
    ...
    */
    function gsw_ct_freezing($sa, $p, $saturation_fraction) {
        $c0  = 0.017947064327968736e0; 	$c1  = -6.076099099929818e0;
        ...
        */
        $a	= 0.014289763856964e0;	/* Note that a = 0.502500117621/35.16504. */
    ...
    */
    function gsw_t_freezing($sa, $p, $saturation_fraction) {
        ...
        $ct_freezing	= self::gsw_ct_freezing($sa,$p,$saturation_fraction);
        $t_freezing	= self::gsw_t_from_ct($sa,$ct_freezing,$p);
    ...
    */
    function gsw_latentheat_melting($sa, $p) {
        $c0  =  3.334265169240710e5; $c1  = -2.789444646733159e0;
    ...
    */
    function gsw_latentheat_evap_ct($sa, $ct) {
        $c0  =  2.499065844825125e6; $c1  = -1.544590633515099e-1;
    ...
    */
    function gsw_latentheat_evap_t($sa, $t) {
        $ct = self::gsw_ct_from_pt($sa,$t);
        return (self::gsw_latentheat_evap_ct($sa,$ct));
    }
    ...
    */
    function gsw_rho_t_exact($sa, $t, $p) {
        ...
        return (1.0e0/self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p));
    }
    ...
    */
    function gsw_pot_rho_t_exact($sa, $t, $p, $p_ref) {
        $pt = self::gsw_pt_from_t($sa,$t,$p,$p_ref);
        return (self::gsw_rho_t_exact($sa,$pt,$p_ref));
    }
    ...
    */
    function gsw_alpha_wrt_t_exact($sa, $t, $p) {
        ...
        return (self::gsw_gibbs($n0,$n1,$n1,$sa,$t,$p)/self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p));
    }
    ...
    */
    function gsw_beta_const_t_exact($sa, $t, $p) {
        ...
        return (-1 * self::gsw_gibbs($n1,$n0,$n1,$sa,$t,$p)/self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p));
    }
    ...
    */
    function gsw_specvol_t_exact($sa, $t, $p) {
        ...
        return (self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p));
    }
    ...
    */
    function gsw_sound_speed_t_exact($sa, $t, $p) {
        ...
        $g_tt	= self::gsw_gibbs($n0,$n2,$n0,$sa,$t,$p);
        $g_tp	= self::gsw_gibbs($n0,$n1,$n1,$sa,$t,$p);

        return (self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p) *
            sqrt($g_tt/($g_tp*$g_tp - $g_tt*self::gsw_gibbs($n0,$n0,$n2,$sa,$t,$p))));
    }
    ...
    */
    function gsw_kappa_t_exact($sa, $t, $p) {
        ...
        return (($g_tp*$g_tp - $g_tt*self::gsw_gibbs($n0,$n0,$n2,$sa,$t,$p)) /
            (self::gsw_gibbs($n0,$n0,$n1,$sa,$t,$p)*$g_tt));
    }
    ...
    */
    function gsw_enthalpy_t_exact($sa, $t, $p) {
        ...
        return (self::gsw_gibbs($n0,$n0,$n0,$sa,$t,$p) -
            ($t+273.15e0)*self::gsw_gibbs($n0,$n1,$n0,$sa,$t,$p));
    }
    ...
    */
    function gsw_cp_t_exact($sa, $t, $p) {
        ...
        return (-1*($t+273.15e0)*self::gsw_gibbs($n0,$n2,$n0,$sa,$t,$p));
    }
    ...
    */
    function gsw_gibbs($ns, $nt, $np, $sa, $t, $p) {
        ...
        if ($ns == 0  && $nt == 0  && $np == 0) {
            ...
            if ($sa > 0.0e0)
                $g08 = $g08 + $x2*(5812.81456626732e0 + 851.226734946706e0*$y)*log($x);
    ...
    *
    * 	input_data	: pointer / reference
    * 	output_data	: pointer / reference
    *
    ...
    */
    ...
    function gsw_add_barrier(&$input_data, $lon, $lat, $long_grid, $lat_grid, $dlong_grid, $dlat_grid, &$output_data) {
        ...
        $above_line = array();
        $k = ''; $nmean = ''; $above_line0 = ''; $kk = '';
        $longs_pan = array(260.0, 272.59, 276.50, 278.65, 280.73, 292.0);
        $lats_pan = array(19.55, 13.97, 9.6, 8.1, 9.33, 3.4);
        $r = ''; $lats_line = ''; $data_mean = '';

        $k			= self::gsw_indx($longs_pan,6,$lon);	/*   the lon/lat point */
        ...
        $k			= self::gsw_indx($longs_pan,6,$long_grid);	/* the 1 & 4 lon/lat points*/
        ...
        $k				= self::gsw_indx($longs_pan,6,$long_grid+$dlong_grid);
        ...
        for ($kk=0; $kk<4; $kk++) {
            if ((abs($input_data[$kk]) <= 100.0) && $above_line0 == $above_line[$kk]) {
                $nmean	= $nmean+1;
        ...
        for ($kk=0; $kk<4; $kk++) {
            if ((abs($input_data[$kk]) >= 1e10) || $above_line0 != $above_line[$kk])
    ...
    */
    /* void */
    function gsw_add_mean(&$data_in, $lon, $lat,&$data_out) {
    ...
    */
    function gsw_indx(&$x, $n, $z) {
        ...
        elseif ($z >= $x[$n-1])
            ...
            $msg = "ERROR in function gsw_indx : out of range\n";
    ...
    */
    function gsw_sa_from_sp_baltic($sp, $lon, $lat) {
        ...
        $xx_right = '';
        $return_value = '';
        $xb_left = array(12.6, 7.0, 26.0);
        $yb_left = array(50.0, 59.0, 69.0);
        $xb_right = array(45.0, 26.0);
        $yb_right = array(50.0, 69.0);

        if ($xb_left[1] < $lon  && $lon < $xb_right[0]  && $yb_left[0] < $lat  && $lat < $yb_left[2]) {
            $xx_left	= self::gsw_xinterp1($yb_left, $xb_left, 3, $lat);
            $xx_right	= self::gsw_xinterp1($yb_right, $xb_right, 2, $lat);
    ...
    */
    function gsw_specvol_sso_0_p($p) {
        ...
        $v01 =  9.998420897506056e2;   $v05 = -6.698001071123802e0;
    ...
    */
    function gsw_enthalpy_sso_0_p($p) {
        ...
        $v36 =  5.419326551148740e-6; $v37 = -2.742185394906099e-5;
        $v41 = -1.105097577149576e-7; $v43 = -1.119011592875110e-10;
        $v47 = -1.200507748551599e-15; $db2pa = self::db2pa;
        $sso = 35.16504e0; $sqrtsso = 5.930011804372737e0;

        $a0 = ''; $a1 = ''; $a2 = ''; $a3 = '';
        $b0 = ''; $b1 = ''; $b2 = ''; $b1sq = '';
        $sqrt_disc = ''; $n = ''; $m = ''; $a = ''; $b = ''; $part = '';
